<img align="left" width="15%" margin="20" style="margin-right:15px ; margin-bottom:15px " src="assets/img/avatar.png">

### Luis Cerda.

---

🎓 **Infrastructure & Cloud Engineer** | 💻 **DevOps Enthusiast** | ⚙️ **SRE Specialist** | 📷 **Macro Photographer**

I'm an IT professional with over 10 years of experience in infrastructure, cloud services, automation, and site reliability. I love building robust, scalable systems that support modern applications and critical operations. I'm also passionate about learning new technologies and applying them to create meaningful change—whether that's optimizing enterprise infrastructure.



---

## 🚀 About Me

- 🧠 Strategic thinker with a strong foundation in IT infrastructure and cloud platforms.
- 🌩️ Deep experience in **AWS**, **Kubernetes**, **Docker**, **Jenkins**, and **Infrastructure as Code**.
- ⚙️ Passionate about **automation**, **system reliability**, and **process optimization**.
- 🧪 Constantly exploring new tech and how it can improve operations or unlock new capabilities.
- 🌱 Currently expanding my backend skills with **Node.js**
-  🌎 Based in Chile
-  🌐 Fluent in Spanish & English (EFSET C2  / TOEIC B2 )
---
### 🧠 Skills

- 🖥️ Infrastructure as Code (IaC)
- ☁️ Cloud Platforms: AWS, Google Cloud, Azure
- 🐳 Docker & Kubernetes
- 📜 Shell Scripting, Python, Git
- 🔄 CI/CD Pipelines (Jenkins)
- 🛡️ System Hardening, Security & Patching
- 📊 Data Analysis & Visualization

---

### 📜 Certifications

- **AWS Certified Cloud Practitioner** *(Aug 2022 – Aug 2025)*  
- **DevOps Foundation® – DevOps Institute** *(Nov 2023 – Nov 2025)*  
- **Google Cloud Fundamentals: Core Infrastructure** *(Feb 2024)*  
- **Essential Google Cloud Infrastructure: Foundation** *(Aug 2024)*  
- **EF SET English Certificate – 77/100 (C2 Proficient)**  
- **TOEIC® Program – English for International Communication B2**

**+ 15+ More Certifications** in Cloud Architecture, CI/CD, Security, Monitoring, DevOps, and Microservices from Capacitación Usach, CORFO, and ACTI Chile.

---

## 🎓 Education

- 🧑‍💻 **Desafío Latam** — Backend JavaScript Development *(2024–2025)*  
- 📊 **Universidad del Desarrollo** — Data Analysis Diploma *(2023–2024)*  
- 🚀 **Universidad de Santiago de Chile** — DevOps Diploma *(2022–2023)*  
- 🏗️ **Duoc UC** — Infrastructure & Technology Platforms Engineering *(2013–2016)*  
  *Graduated — Specialized in managing infrastructure services and enterprise platforms.*

---

## 🌍 Let's Connect

- [LinkedIn](https://www.linkedin.com/in/luis-cerda-s/)
- [GitHub](https://github.com/khyron)
- [Instagram](https://www.instagram.com/macrocl/) 


